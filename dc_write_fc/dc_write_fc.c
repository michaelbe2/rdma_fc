...
typedef struct {
    ...
    int                       conn_timeout_ms;
    size_t                    rdma_write_size;
    unsigned                  num_iterations;
    size_t                    max_outstanding_writes;
...
typedef struct {
    ...
    unsigned long             num_outstanding_writes;
} test_context_t;
...
options_t g_options = {
    ...
    .rdma_write_size        = 1024 * 1024,
    .num_iterations        = 1000,
    .max_outstanding_writes = 8,
    .max_write_size         = 32768,
    .tx_queue_len          = 128,
    .rx_queue_len          = 128,
    .max_send_sge          = 1,
    .max_recv_sge          = 1,
...
static void usage(const options_t *defaults) {
    ...
    printf("   -r <size>      Maximal size of a single rdma write (%zu)\n", defaults->max_write_size);
    printf("   -o <num>       Maximal number of outstanding writes (%zu)\n", defaults->max_outstanding_writes);
}
...
static int parse_opts(int argc, char **argv) {
    ...
    while ( (c = getopt(argc, argv, "hp:n:vt:i:r:o:G:T:H:S:")) != -1 ) {
        ...
        case 'i':
            ...
            break;
        case 'r':
            g_options.max_write_size = atol(optarg);
            break;
        case 'o':
            g_options.max_outstanding_writes = atol(optarg);
            break;
        case 'G':
...
static int init_transport(struct rdma_cm_event *event)
{
    ...
    ret = init_buffer(event->id->pd,
                      g_options.num_connections * g_options.rdma_write_size,
                      &g_test.rdma_buf);
    if (ret) {
        return ret;
    }
                          ...
                          conn_priv_t *priv, unsigned conn_index)
{
    ...
    priv->virt_addr           = (uint64_t)g_test.rdma_buf.ptr +
                                (conn_index * g_options.rdma_write_size);
...
static int poll_cq()
{
    ...
    if (ret < 0) {
        ...
        case IBV_WC_RDMA_WRITE:
            /* Outgoing RDMA_WRITE was completed */
            LOG_TRACE("RDMA_WRITE completion wr_id %ld", wc.wr_id);
            --g_test.num_outstanding_writes;
            dc_send_completion(&wc);
            break;
        default:
...
/* post rdma_write, set *posted to 1 if the operation was posted, 0 if no
 * available QP is found
 */
static int post_rdma_write(connection_t *conn, size_t offset, size_t length,
                          int *posted)
{
    ...
    LOG_TRACE("post_rdma_write: ibv_wr_start: qpex = %p\n", g_test.dcis_ex[dci]);
    ibv_wr_start(g_test.dcis_ex[dci]);

    g_test.dcis_ex[dci]->wr_id = (uint64_t)dci;
    g_test.dcis_ex[dci]->wr_flags = IBV_SEND_SIGNALED;

    LOG_TRACE("post_rdma_write: ibv_wr_rdma_write: wr_id=0x%lx, qpex=%p, rkey=0x%x, remote_addr=%p\n",
              g_test.dcis_ex[dci]->wr_id, g_test.dcis_ex[dci], conn->rkey, (void*)remote_addr);
    ibv_wr_rdma_write(g_test.dcis_ex[dci], conn->rkey, remote_addr);

    LOG_TRACE("post_rdma_write: mlx5dv_wr_set_dc_addr: mqpex=%p, ah=%p, rem_dctn=0x%06x\n",
               g_test.m_dcis_ex[dci], conn->dc_ah, conn->remote_dctn);
    mlx5dv_wr_set_dc_addr(g_test.m_dcis_ex[dci], conn->dc_ah, conn->remote_dctn, DC_KEY);

    LOG_TRACE("post_rdma_write: ibv_wr_set_sge: qpex=%p, lkey=0x%x, local_buf=%p, size=%lu\n",
    ...
    LOG_TRACE("post_rdma_write: ibv_wr_complete: qpex=%p\n", g_test.dcis_ex[dci]);
    ret = ibv_wr_complete(g_test.dcis_ex[dci]);
    if (ret) {
        LOG_ERROR("post_rdma_write: ibv_wr_complete failed (error=%d)\n", ret);
        return -1;
    }
...
static int do_rdma_writes()
{
    ...
    g_test.num_outstanding_writes = 0;
    ...
    for (iter = 0; iter < g_options.num_iterations; ++iter) {
        ...
        for (i = 0; i < g_test.num_conns; ++i) {
            g_test.conns[i].write_offset = 0;
            list_add_tail(&sched, &g_test.conns[i].list);
        }

        /* As long as not all write operations are completed, and the list is not
         * empty, continue working
         */
        while (g_test.num_outstanding_writes || !list_is_empty(&sched)) {

            //MB:
            if (!list_is_empty(&sched) &&
                (g_test.num_outstanding_writes == g_options.max_outstanding_writes)) {
                LOG_TRACE("num_outstanding_writes (%ld) reached maximum\n",
                          g_test.num_outstanding_writes);
            }
            /* If there are more connections with pending data, and we have not
             * exceeded the total number of outstanding writes, post the next
             * write operation
             */
            if (!list_is_empty(&sched) &&
                (g_test.num_outstanding_writes < g_options.max_outstanding_writes))
            {
                LOG_TRACE("num_outstanding_writes = %ld\n", g_test.num_outstanding_writes);
                /* take the first connection from the schedule queue */
                conn = list_extract_head(&sched, connection_t, list);

                /* see how many bytes are left to write */
                size = MIN(g_options.max_write_size,
                           g_options.rdma_write_size - conn->write_offset);
                assert(size > 0);

                /* issue rdma write on the next segment */
                posted = 0;
                ret = post_rdma_write(conn, conn->write_offset, size, &posted);
                if (ret < 0) {
                    return ret;
                }
                ...
                if (posted) {
                     ...
                     * the schedule queue
                     */
                    if (conn->write_offset < g_options.rdma_write_size) {
    ...
    for (i = 0; i < g_test.num_conns; ++i) {
        assert(g_test.conns[i].write_offset == g_options.rdma_write_size);
    }

    /* Calculate and report total write bandwidth */
    sec_elapsed = (tv_end.tv_sec - tv_start.tv_sec) +
                  (tv_end.tv_usec - tv_start.tv_usec) * 1e-6;
    bytes_transferred = g_test.num_conns * 
                        g_options.num_iterations * g_options.rdma_write_size;
    LOG_INFO("Total write bandwidth: %.2f MB/s",
